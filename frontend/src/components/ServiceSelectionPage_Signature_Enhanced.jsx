...
import { Globe, ChevronRight, Shield, Zap, Copy, CheckCircle, Clock, AlertTriangle } from 'lucide-react';
...
function ServiceSelectionPage() {
  ...
  const handleSelect = async (serviceId) => {
    ...
    try {
      ...
      if (result.success) {
        // Backend returns: { success: true, serviceId: "lootlab", randomId: "8kL96Hj" }
        ...
        navigate(`/${result.serviceId}-${result.randomId}`);
      }
    } catch (error) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4 mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="text-blue-400 font-semibold text-sm">Auto-Cleanup Activated</h4>
              <p className="text-slate-400 text-xs">Key hết hạn sẽ tự động xóa khỏi localStorage</p>
            </div>
          </div>
        </div>
