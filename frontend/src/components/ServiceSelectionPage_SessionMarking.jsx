...
import { Globe, ChevronRight, Shield, Zap, Copy, CheckCircle, Clock, AlertTriangle, Loader2 } from 'lucide-react';
...
function ServiceSelectionPage() {
  ...
  const handleSelect = async (serviceId) => {
    ...
    try {
      // Generate random ID first
      const randomId = Math.random().toString(36).substring(2, 10).toLowerCase();
      
      // Call API to MARK SESSION in database FIRST
      const apiBaseUrl = import.meta.env.VITE_API_BASE_URL || 'https://khoablabla-backend.hf.space';
      const response = await fetch(`${apiBaseUrl}/api/mark-session`, {
        ...
        body: JSON.stringify({ 
          serviceId, 
          randomId,
          ipAddress: 'client', // Will be updated by backend
      ...
      if (result.success) {
        ...
        console.log(`[ServiceSelection] Session marked: ${serviceId}-${randomId}`);
      ...
      <div className="max-w-4xl mx-auto">
          ...
          {services.map((service) => {
            ...
            return (
              <button
                ...
                disabled={isMarkingSession || generatedLink !== ''}
                ...
                } ${(isMarkingSession || generatedLink) ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                <div className="flex flex-col items-center space-y-4">
                  ...
                  {isMarkingSession && selectedService === service.id ? (
          ...
          <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl border border-slate-700 p-6 mb-8 animate-fade-in">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-white">Phiên đã đánh dấu!</h3>
        ...
        <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4 mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="text-blue-400 font-semibold text-sm">Session Marking + Auto-Cleanup</h4>
              <p className="text-slate-400 text-xs">Phiên được đánh dấu trong Database trước khi navigate</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-800/30 rounded-xl border border-slate-700/50 p-6">
          ...
          <ol className="space-y-2 text-slate-400 text-sm list-decimal list-inside">
            <li>Chọn dịch vụ → Đánh dấu phiên trong Database</li>
